...
class BeginnerSectionEditorController extends Controller
{
    private array $tabs = [
        'home' => 'Home',
        'watch' => 'Watch',
    ...
    public function edit(Request $request, AppSection $appSection): View
    {
        ...
        $currentTab = $this->cleanTab($request->query('tab', $appSection->tab_key));
        $returnTo = $this->sanitizeReturnTo($request->query('return'));
        $returnUrl = $this->returnUrl($returnTo, $currentTab);

        return view('admin.beginner.sections.edit', [
            'section' => $appSection,
            'tabs' => $this->tabs,
            ...
            'meta' => $this->safeArray($appSection->meta_json),
    ...
    public function update(Request $request, AppSection $appSection): RedirectResponse
    {
        $activeAppId = (int) (ActiveApp::ensureId() ?? 0);
        abort_unless($activeAppId > 0, 404);
        abort_unless((int) $appSection->app_id === $activeAppId, 404);
        ...
        $existingMeta = $this->safeArray($appSection->meta_json);
        ...
        $appSection->fill([
            ...
            'meta_json' => $this->placementMetaFromValidated($validated, $existingMeta),
        ]);
    ...
    public function move(Request $request, AppSection $appSection, string $direction): RedirectResponse
    {
        $activeAppId = (int) (ActiveApp::ensureId() ?? 0);
        abort_unless($activeAppId > 0, 404);
        abort_unless((int) $appSection->app_id === $activeAppId, 404);
        ...
        $currentOrder = (int) $appSection->sort_order;
        $swapQuery = AppSection::query()
            ->where('app_id', $activeAppId)
            ->where('tab_key', $appSection->tab_key)
            ->where('id', '!=', $appSection->id);

        $swapSection = $direction === 'up'
            ? $swapQuery->where('sort_order', '<=', $currentOrder)->orderByDesc('sort_order')->orderByDesc('id')->first()
            : $swapQuery->where('sort_order', '>=', $currentOrder)->orderBy('sort_order')->orderBy('id')->first();

        if (! $swapSection) {
            return back()->with('status', 'Section is already in that position.');
        }

        DB::transaction(function () use ($appSection, $swapSection, $currentOrder): void {
            $appSection->sort_order = (int) $swapSection->sort_order;
            $appSection->save();
    ...
    public function destroy(Request $request, AppSection $appSection): RedirectResponse
    {
        $activeAppId = (int) (ActiveApp::ensureId() ?? 0);
        abort_unless($activeAppId > 0, 404);
        abort_unless((int) $appSection->app_id === $activeAppId, 404);

        DB::transaction(function () use ($appSection): void {
            $appSection->items()->delete();
            $appSection->delete();
        });
